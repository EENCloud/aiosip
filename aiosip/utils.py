...
# RFC 3261 section 8.1.1.7: every branch generated by an RFC 3261 compliant UA
# must start with this magic cookie.
...
def gen_str(length=10, letters=string.ascii_letters+string.digits):
    return "".join([secrets.choice(letters) for n in range(length)])


def gen_branch(length=16, letters=string.ascii_letters+string.digits):
    """Generate a Via branch parameter (RFC 3261 section 8.1.1.7).

    The branch identifies a transaction and must be unique across space and
    time for every request the UA sends (CANCEL and the ACK for a non-2xx
    response excepted, they reuse the branch of the request they refer to).
    """
    return "".join((BRANCH_MAGIC_COOKIE, gen_str(length=length, letters=letters)))


def replace_branch(via, branch=None):
    """Return the Via header value(s) with the top Via's branch replaced.

    ``via`` is the value stored in ``message.headers['Via']``: either a str or,
    for several Via headers, a list of str. A new branch is generated when
    ``branch`` is None.
    """
    if branch is None:
        branch = gen_branch()
    if isinstance(via, list):
        return [replace_branch(via[0], branch)] + list(via[1:])
    new_via, count = BRANCH_RE.subn(r'\g<1>' + branch, via, count=1)
    if count == 0:
        new_via = '{};branch={}'.format(via, branch)
    return new_via
